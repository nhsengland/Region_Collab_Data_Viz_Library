...
blues <- c("#003087", "#005EB8", "#0072CE", "#41B6E6", "#00A9CE")

color_scale <- setNames(blues, unique(Attends$org_code))


Attends$org_code <- factor(Attends$org_code, levels = (unique(Attends$org_code)))

plot_ly(data = Attends |> group_by(period, org_code) |>  summarise(count = n()) |> 
          mutate(org_code = as.character(org_code)) |> ungroup(),
        x = ~period,
        y = ~count,
        color = ~org_code,
        colors = color_scale,
