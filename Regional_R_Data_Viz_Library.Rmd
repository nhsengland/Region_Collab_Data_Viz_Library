...
plot_ly(multiple_line_data, x = ~period, y = ~multiple_line_data$attend_rth, name = 'RTH', type = 'scatter', mode = 'lines',  ##Plot the first line as dots
...
plot_ly(Attends, x = ~attendances, y = ~admissions, type = 'scatter', mode = 'markers', color = ~org_code,
  ...
  layout(title = 'Scatter plot',                           ##set titles and axis labels
         ...
         legend = list()) %>% 
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "select2d", "lasso2d", "toImage",   
                                    "pan2d", "autoScale2d", "resetScale2d", "zoom2d"))  ##remove any unnecessary functions of plotly chart
```
...
sankey <- htmlwidgets::prependContent(sankey, htmltools::tags$h1("Attends by Age"))
