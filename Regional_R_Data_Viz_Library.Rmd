...
#Filter initial dataset to just select one trust
line_df <- Attends %>%
  filter(org_code=="RXQ")
  
line_df <- line_df[order(line_df$period),] #Order the data frame by the date field in ascending order. This is required for the chart to plot properly
...
plot_ly(line_df, x = ~period, y = ~attendances, type = 'scatter', mode = 'lines', 
   ...
   layout(title = "Type 1 attendances - Bucks Healthcare", ##set titles and axis labels
         ...
         yaxis = list (title ="Attendances", range = c(0, max(line_df$attendances) * 1.1))) %>%  #To align with the GGPlot set the y axis range from 0 to 1*1 times the max value
