...
line_df <- line_df[order(line_df$period),] #Order the data frame by the date in ascending order - required for the chart to plot properly
line_df <- line_df[order(line_df$period),] # Order the data frame by the period field in ascending order - required for the plotly chart to plot properly

# Plotly Chart
plot_ly(line_df, x = ~period, y = ~attendances, type = 'scatter', mode = 'lines',  ##Plot the first line as dots in NHS Grey
  ...
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "select2d", "lasso2d", "toImage",   ##remove any unnecessary functions of plotly chart
         ...
         yaxis = list (title ="Attendances", range = c(0, max(line_df$attendances) * 1.1)) %>% # In line with GG Plot version, set the y axis range from 0 to slightly higher than the max value
...
For more info check:- https://github.com/nhs-r-community/NHSRplotthedots
...
#Remove orgs that still appear in chart despite being filtered out!

AttendsAll$org_code <- as.character(AttendsAll$org_code)

#Restructure data so it works for stacked bar chart
Attends_wide <-Attends |> 
...
Attends_wide_per[, -c(1,2)] <- Attends_wide[, -c(1,2)] / rowSums(Attends_wide[, -c(1,2)]) #* 100
...
plot_ly(Attends, x = ~attendances, y = ~admissions, type = 'scatter', mode = 'markers', color = ~org_code,
  ...
  layout(title = 'Scatter plot',                           ##set titles and axis labels
         ...
         legend = list()) %>% 
  config(modeBarButtonsToRemove = c("zoomIn2d", "zoomOut2d", "select2d", "lasso2d", "toImage",   
                                    "pan2d", "autoScale2d", "resetScale2d", "zoom2d"))  ##remove any unnecessary functions of 
```
...
## Sankey Diagram {.tabset .tabset-fade}
...
##Make the sankey network , Iterations=0  means that the the data is displayed according to how it is sorted in the initial df. starting owith the source node 
